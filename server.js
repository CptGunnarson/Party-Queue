...
app.get("/search", async (req, res) => {
  if (!access_token) {
    return res.status(401).json({ error: "Spotify nicht verbunden." });
  }
...
app.post("/add", async (req, res) => {
  if (!access_token) {
    return res.status(401).json({ error: "Token fehlt oder Spotify nicht verbunden." });
  }

  const { uri } = req.body;
  if (!uri) {
    return res.status(400).json({ error: "Song-URI fehlt." });
  }
...
app.listen(PORT, () => {
  ...
  console.log(`👉 Öffne ${REDIRECT_URI} um Spotify zu verbinden`);
});
